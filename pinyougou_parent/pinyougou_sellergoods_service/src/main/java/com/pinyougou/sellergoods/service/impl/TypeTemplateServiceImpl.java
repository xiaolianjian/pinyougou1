...
public class TypeTemplateServiceImpl implements TypeTemplateService {
	...
	private void saveToRedis() {
		...
		for (TbTypeTemplate template : all) {
			...
			redisTemplate.boundHashOps("brandIds").put(template.getId(), brandIds);

			//缓存规格列表
			List<Map> specIds = findSpecList(template.getId());
			redisTemplate.boundHashOps("specIds").put(template.getId(), specIds);
		}
	}
